...
## v1.7.1 物語の複数ルート予測と中断しない Studio コラボレーション

InkOS 1.7.1 では、次章を書く前に複数の未来を比較できます。Studio Chat は現在の正史から互いに独立した候補ルートを生成し、章のビート、人物の決断、予想される変化、リスク、作者意図との一致度を横並びで表示します。ルートを採用しても候補計画が保存されるだけで、本文、アウトライン、物語状態が先に変更されることはありません。

- **物語の複数ルート予測**：Studio Chat または CLI から 2-5 本の非正史ルートを作成、再検証、選択できます。正史が変わると古い予測は stale になります。
- **執筆中も会話を継続**：章の生成はバックグラウンドで進み、長時間タスクが Chat 全体を占有しません。更新後も進捗と完了状態を復元できます。
- **タスク復旧の信頼性向上**：失敗メッセージの再試行、セッション削除時のタスク中止、再起動後の zombie 状態の整理、古い Play 選択肢の復活防止に対応します。
- **創作データをより安全に**：書籍全体のバックアップ / 復元、最新章削除時の状態ロールバック、部分編集後の章インデックス文字数同期を追加しました。
- **追跡可能なツール結果**：直接終了するツール呼び出しも更新後に完全なカードとして復元され、完了判定は実際の tool result とファイルに基づきます。
