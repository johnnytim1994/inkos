...
## v1.7.1 - 剧情多线推演与不中断的 Studio 协作

InkOS 1.7.1 让长篇创作可以先比较未来，再决定下一章怎么写。Studio Chat 会基于当前正史生成多条彼此隔离的候选分支，横向展示章节节拍、人物决定、预计变化、风险和作者意图匹配度；采用分支只保存候选计划，不会提前修改正文、大纲或故事状态。

- **剧情多线推演**：Studio Chat 与 CLI 均可创建、核验和选择 2-5 条非正史未来分支；正史变化后旧推演会自动标记为过期。
- **写作时仍可继续聊天**：写下一章等生产任务在后台推进，对话不再被长任务占满；刷新页面后仍可恢复进度与终态。
- **任务恢复更可靠**：支持失败消息重试、会话删除时中止任务、服务重启后清理僵尸状态，并防止旧快照或旧 Play 选项复活。
- **创作数据更安全**：新增整书备份 / 恢复、最新章节删除与状态回滚；章节局部编辑会同步更新索引字数。
- **工具结果可追溯**：直接终态工具调用在刷新后也能恢复为完整卡片，系统仍只根据真实工具结果和文件判定完成。
